...
data.country.forEach(country => {
  ...
  country.item.forEach(item => {
    ...
    incbtn.addEventListener('click', function () {
      ...
      cart[item.name] = { price: item.price, quantity: number };
...
addToCartBtn.style.bottom = '10px';  // 20px from the bottom
addToCartBtn.style.left = '50%';
addToCartBtn.style.transform = 'translateX(-50%)';
addToCartBtn.style.padding = '12px 25px';
addToCartBtn.style.fontSize = '18px';  // Increased font size
addToCartBtn.style.background = 'linear-gradient(135deg, #2A9D8F, #264653)';
addToCartBtn.style.color = 'white';
addToCartBtn.style.borderRadius = '50px';
addToCartBtn.style.border = 'none';
addToCartBtn.style.cursor = 'pointer';  // Changes cursor to a pointer
addToCartBtn.style.transition = 'all 0.3s ease';  // Smooth transition effect
addToCartBtn.style.boxShadow = '0 4px 12px rgba(0, 0, 0, 0.4)';  // Adds shadow for depth

// Hover effect for interactivity
addToCartBtn.onmouseover = function() {
  addToCartBtn.style.background = 'linear-gradient(135deg, #264653, #2A9D8F)';
  addToCartBtn.style.transform = 'translateX(-50%) scale(1.05)';  // Slightly increases the size
};

addToCartBtn.onmouseout = function() {
  addToCartBtn.style.background = 'linear-gradient(135deg, #2A9D8F, #264653)';
  addToCartBtn.style.transform = 'translateX(-50%)';  // Resets the scale
};
